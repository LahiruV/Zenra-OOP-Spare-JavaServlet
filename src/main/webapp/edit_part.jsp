<%@ page import="java.util.List, com.vehicle.parts.model.Part, com.vehicle.parts.dao.PartDAO, com.vehicle.parts.dao.SupplierDAO, com.vehicle.parts.model.Supplier" %>
<%
...
<div class="flex-1 flex flex-col">
    <header class="bg-white shadow px-6 py-4">
        <h1 class="text-xl font-bold">Edit Spare Part</h1>
    </header>

    <main class="flex justify-center items-start p-6">
        <div class="bg-white p-8 rounded-lg shadow-lg w-full max-w-md">
            <form action="updatePart" method="post" enctype="multipart/form-data" class="space-y-4">
                <input type="hidden" name="id" value="<%= part.getId() %>" />
                <input type="text" name="name" value="<%= part.getName() %>" required class="w-full border px-4 py-2 rounded" />
                <textarea name="description" class="w-full border px-4 py-2 rounded"><%= part.getDescription() %></textarea>
                <input type="number" step="0.01" name="price" value="<%= part.getPrice() %>" required class="w-full border px-4 py-2 rounded" />
                <input type="number" name="quantity" value="<%= part.getQuantity() %>" required class="w-full border px-4 py-2 rounded" />

                <select name="supplier_id" required class="w-full border px-4 py-2 rounded">
                    <% for (Supplier s : suppliers) { %>
                        <option value="<%= s.getId() %>" <%= s.getId() == part.getSupplierId() ? "selected" : "" %>>
                            <%= s.getName() %>
                        </option>
                    <% } %>
                </select>
                ...
                <div class="text-center">
                    <img src="data:image/jpeg;base64,<%= part.getImage() %>" class="w-40 h-auto mx-auto rounded mt-2" />
                </div>

                <button type="submit" class="w-full bg-green-600 text-white py-2 rounded hover:bg-green-700">Update Spare Part</button>
            </form>
        </div>
    </main>
</div>
